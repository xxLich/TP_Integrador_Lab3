...
BEGIN
    ...
    -- Evita recursividad infinita: solo ejecutar si nivel de trigger es 1
    IF TRIGGER_NESTLEVEL() > 1
        RETURN;

    -- Actualizar Estado a 0 si Stock=0 y Estado no es 0
    UPDATE C
    SET C.Estado = 0
    FROM Componentes C
    ...
    -- Actualizar Estado a 1 si Stock > 0 y Estado no es 1
    UPDATE C
    SET C.Estado = 1
    FROM Componentes C
    INNER JOIN inserted I ON C.IDComponente = I.IDComponente
...
/* PRUEBA DE LOS ESTADOS 
UPDATE Componentes
SET Stock = 0
WHERE IDComponente = 1;

SELECT * FROM Componentes WHERE IDComponente = 1;
